...
public class TestConnector implements PoolableConnector, AuthenticateOp, CreateOp, DeleteOp, ResolveUsernameOp,
     ...
     * password. The {@link org.identityconnectors.framework.spi.Connector}
     * developer is expected to attempt to authenticate these credentials
     * natively. If the authentication fails the developer should throw a type
     * of {@link RuntimeException} either {@link IllegalArgumentException} or
     * if a native exception is available and if its of type
     * {@link RuntimeException} simple throw it. If the native exception is not
     * a {@link RuntimeException} wrap it in one and throw it. This will
     ...
     * most common is
     * {@link org.identityconnectors.framework.common.exceptions.InvalidPasswordException}.
     *
     * @param   objectClass
     ...
     *          If the caller passes {@code null}, the framework will convert
     *          this into an empty set of options, so that the SPI implementor
     *          need not worry about handling {@code null} explicitly.
     *
     * @return  The uid of the account that was used to authenticate
     *
     * @throws  RuntimeException
     *          If native authentication fails.
     *          If a native exception is available, it will be thrown instead.
     */
    @Override
     ...
     * operations can be invoked on it (for newBuilder, an implementation would
     * check that the connector's physical connection to the resource has not
     * timed out).
     * <p>
     * The major difference between this method and
     * {@link org.identityconnectors.framework.spi.operations.TestOp#test()} is
     * that this method must do only the minimum that is necessary to check
     * that the connector is still alive. <code>TestOp.test()</code> does a
     * more thorough check of the environment specified in the Configuration,
     * and can therefore be much slower.
     * <p>
     * This method can be called often. Implementations should do their best to
     * keep this method fast and low-overhead.
     *
     * @throws  RuntimeException
     ...
     * {@link #init(org.identityconnectors.framework.spi.Configuration)}.
     *
     * @return The configuration that was passed to
     *         {@link #init(org.identityconnectors.framework.spi.Configuration)}.
     */
    ...
    /**
     * Initialize the connector with its configuration. For newBuilder in a
     * JDBC {@link org.identityconnectors.framework.spi.Connector} this would
     * include the database URL, password, and user.
     *
     * @param cfg
     *        newBuilder of the
     *        {@link org.identityconnectors.framework.spi.Configuration} object
     *        implemented by the
     *        {@link org.identityconnectors.framework.spi.Connector} developer
     *        and populated with information in order to initialize the
     ...
     * {@link org.identityconnectors.framework.spi.Connector} uses.
     */
    @Override
    public void dispose() {
    ...
    /**
     * The {@link org.identityconnectors.framework.spi.Connector} developer is
     * responsible for taking the attributes given (which always includes the
     * {@link org.identityconnectors.framework.common.objects.ObjectClass}) and
     * create an object and its
     * {@link org.identityconnectors.framework.common.objects.Uid}.
     * The {@link org.identityconnectors.framework.spi.Connector} developer
     * must return the
     * {@link org.identityconnectors.framework.common.objects.Uid} so that the
     * caller can refer to the created object.
     * <p>
     * *Note: There will never be a
     * {@link org.identityconnectors.framework.common.objects.Uid} passed in
     * with the attribute set for this method. If the resource supports some
     * sort of mutable
     * {@link org.identityconnectors.framework.common.objects.Uid}, you should
     * create your own resource-specific attribute for it, such as
     * <I>unix_uid</I>.
     *
     * @param   attrs
     ...
     *          attribute and
     *          {@link org.identityconnectors.framework.common.objects.Name}
     *          attribute.
     * @param   options
     *          Additional options that impact the way this operation is run.
     *          If the caller passes {@code null}, the framework will convert
     *          this into an empty set of options, so the implementor of the
     *          SPI need not worry about this ever being {@code null}.
     * @return  The unique id for the object that is created. For
     *          {@code newBuilder} in LDAP this would be the {@code 'dn'}, for
     *          a database this would be the primary key, and for
     *          {@code 'ActiveDirectory'} this would be the GUID.
     */
    @Override
    ...
    /**
     * The {@link org.identityconnectors.framework.spi.Connector} developer is
     * responsible for calling the native delete methods to remove the object
     ...
     *          Iff the {@link org.identityconnectors.framework.common.objects.Uid}
     ...
     * {@link org.identityconnectors.framework.common.objects.Uid} based on its
     * username. This is a companion to the simple
     * {@link org.identityconnectors.framework.spi.operations.AuthenticateOp authentication}.
     * The difference is that this method does not have a password parameter
     * and does not try to authenticate the credentials; instead, it returns
     * the {@link org.identityconnectors.framework.common.objects.Uid}
     * corresponding to the username. Implementations method must, however,
     * validate the username (i.e., they must throw and exception if the
     * username does not correspond to an existing object).
     * <p>
     ...
     * The developer is of course encourage to try and throw the most
     * informative exception as possible. In that regards there are several
     * exceptions provided in the exceptions package. For {@code newBuilder}
     * one of the most common is
     * {@link org.identityconnectors.framework.common.exceptions.UnknownUidException}.
     *
     * @since   1.1
     *
     * @param   objectClass
     *          The object class to resolve the username for. Will typically be
     *          an account. Will not be {@code null}.
     * @param   username
     ...
     *          Additional options that impact the way this operation is run.
     *          If the caller passes {@code null}, the framework will convert
     *          this into an empty set of options, so the impelementor of the
     *          SPI need not worry about this ever being {@code null}.
     *
     * @return  The UID of the object corresponding to the username.
     *
     * @throws  RuntimeException
     ...
     * {@link org.identityconnectors.framework.spi.Connector} supports. This
     * method is considered an operation since determining supported objects
     * may require configuration information and allows this determination to
     * be dynamic.
     * <p>
     ...
     * <I>unix_uid</I>.
     *
     * @return The Basic schema supported by this
     *         {@link org.identityconnectors.framework.spi.Connector}.
     */
    @Override
     ...
     * @return  The result of the script. The return type must be a type that
     *          the framework supports for serialization. See
     ...
     *
     * @param   objectClass
     *          The object class for the search. Will never be {@code null}.
     * @param   options
     *          Additional options that impact the way this operation is run.
     *          If the caller passes {@code null}, the framework will convert
     *          this into an empty set of options, so an implementor of the SPI
     *          need not worry about this ever being {@code null}.
     *
     * @return  A filter translator. This must not be {@code null}.
     *          A {@code null} return value will cause the API
     ...
     *      org.identityconnectors.framework.common.objects.OperationOptions) FilterTranslator}
     ...
     *   org.identityconnectors.framework.common.objects.OperationOptions)} SearchApiOp}.
     * If the <code>FilterTranslator</code> produces more than one native
     * query, then ConnectorFacade will automatically merge the results from
     * each query and eliminate any duplicates. NOTE that this implies an in
     * memory data structure that holds a set of UID values, so memory usage in
     * the event of multiple queries will be O(N) where N is the number of
     ...
     *          The object class for the search. Will never be {@code null}.
     * @param   query
     *          The native query to run. A value of {@code null} means "return every
     *          newBuilder of the given object class".
     * @param   handler
     *          The handler to which results must be returned.
     * @param   options
     *          Additional options that impact the way this operation is run.
     *          If the caller passes {@code null}, the framework will convert
     *          this into an empty set of options, so an implementor of the SPI
     *          need not guard against this being {@code null}.
     */
    @Override
    ...
    /**
     * Request synchronization events -- i.e., native changes to target
     * objects.
     * <p>
     * This method will call the specified
     * {@linkplain org.identityconnectors.framework.common.objects.SyncResultsHandler#handle(org.identityconnectors.framework.common.objects.SyncDelta)} handler}
     * once to pass back each matching
     * {@linkplain org.identityconnectors.framework.common.objects.SyncDelta synchronization event}.
     * Once this method returns, this method will no longer invoke the
     * specified handler.
     * <p>
     * Each
     ...
     * reading events <i>starting from that point in the event stream</i>.
     * In typical usage, a client will save the token from the final
     * synchronization event that was received from one invocation of this
     * {@code sync()} method and then pass that token into that client's
     * {@code buildNext} call to this {@code sync()} method. This allows a
     * client to "pick up where he left off" in receiving synchronization
     * events. However, a client can pass the token from <i>any</i>
     * synchronization event into a subsequent invocation of this
     * {@code sync()} method. This will return synchronization events (that
     * represent native changes that occurred) immediately subsequent to the
     * event from which the client obtained the token.
     * <p>
     * A client that wants to read synchronization events "starting now"
     * can call
     * {@link #getLatestSyncToken(org.identityconnectors.framework.common.objects.ObjectClass)}
     * and then pass that token into this {@code sync()} method.
     *
     * @param   objClass
     *          The class of object for which to return synchronization events.
     ...
     *          objects, each of which contains a token.
     *          Should be {@code null} if this is the client's first call to
     *          the {@code sync()} method for this connector.
     * @param   handler
     *          The result handler. Must not be {@code null}.
     * @param   options
     *          Options that affect the way this operation is run.
     *          If the caller passes {@code null}, the framework will convert
     *          this into an empty set of options, so an implementation need
     *          not guard against this being {@code null}.
     *
     * @throws  IllegalArgumentException
     *          If {@code objClass} or {@code handler} is {@code null} or if
     ...
     * <p>
     * An application that wants to receive synchronization events "starting
     * now" -- i.e., wants to receive only native changes that occur after this
     * method is called -- should call this method and then pass the resulting
     * token into
     * {@linkplain #sync(org.identityconnectors.framework.common.objects.ObjectClass,
     ...
     * org.identityconnectors.framework.common.objects.OperationOptions)} the
     * sync() method}.
     *
     * @param   objClass
     *          The class of object for which to find the most recent
     *          synchronization event (if any). Must not be {@code null}.
     *
     * @return  A token if synchronization events exist; otherwise {@code null}.
     *
     * @throws  IllegalArgumentException
     *          If {@code objClass} is {@code null} or is invalid.
     */
    @Override
     ...
     *
     * @param   objClass
     *          The type of object to modify. Will never be {@code null}.
     * @param   uid
     *          The UID of the object to modify. Will never be {@code null}.
     * @param   valuesToAdd
     *          The set of
     *          {@link org.identityconnectors.framework.common.objects.Attribute}
     *          deltas. The values for the attributes in this set represent the
     *          values to add to attributes in the object. This set will never
     *          include
     ...
     *          of the updated object in case the update changes the formation
     ...
     * any value that matches one of the values of the attribute from the input
     * set.
     * <p>
     * NOTE that this does not specify how to handle unmatched values.
     * The general assumption for an attribute of a {@code ConnectorObject}
     * is that the values for an attribute are merely
     * <i>representational state</i>. Therefore, the implementer should simply
     * ignore any provided value
     ...
     *          {@link org.identityconnectors.framework.common.objects.Attribute}
     *          deltas. The values for the attributes in this set represent the
     *          values to remove from attributes in the object. This set will
     *          never include
     *          {@link org.identityconnectors.framework.common.objects.OperationalAttributes operational attributes}.
     *          Will never be {@code null}.
     * @param   options
     ...
     * @throws  org.identityconnectors.framework.common.exceptions.UnknownUidException
     *          Iff the
     *          {@link org.identityconnectors.framework.common.objects.Uid} does
     ...
     *          The set of new
     *          {@link org.identityconnectors.framework.common.objects.Attribute}.
     *          The values in this set represent the new, merged values to be
     *          applied to the object. This set may also include
     *          {@link org.identityconnectors.framework.common.objects.OperationalAttributes operational attributes}.
     *          Will never be {@code null}.
     * @param   options
     *          Additional options that impact the way this operation is run.
