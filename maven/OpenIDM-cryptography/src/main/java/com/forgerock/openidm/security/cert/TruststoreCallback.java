 ...
 * @since 1.0.0
 */
public class TruststoreCallback implements CallbackHandler {

    public static final String code_id = "$Id$";
    private static final Trace logger = TraceManager.getTrace(TruststoreCallback.class);
    ...
    public void handle(Callback[] clbcks) throws IOException, UnsupportedCallbackException {
        for (int i = 0; i < clbcks.length; i++) {
            if (clbcks[i] instanceof KeyStoreCallback) {
                ...
                cb.setKeystore(KeyStoreManager.getTrustStore());
            }
        }
    }
}
